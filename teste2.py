...
df = pd.read_excel('c:/Users/Alan/Documents/magic/Magic.xlsx')
...
for i in range(len(df["nome_ingles"])):
    elemento_entrada = driver.find_element_by_id("mainsearch")
    elemento_entrada.send_keys(df["nome_ingles"][i])
    ...
    if df["busca_carta"][i] == 2:
        carta_pagina = driver.find_element_by_xpath("//*[contains(text(),elemento_entrada)]")
        if carta_pagina == elemento_entrada:
            carta = driver.find_element_by_xpath("/html/body/main/div[2]/div[4]/div/div[1]/div/div[1]/a/img").click()
