...
def pegar_dados_cartas(page, nome_pt_fallback, nome_en_fallback):
    ...
    def extrair_dados_da_tela_atual():
        ...
        try:
            edicao = page.locator('div.name-edition').inner_text().replace('\n', ' ').strip()
        except Exception:
            edicao = "Edição Única"
        ...
        # 3. Raridade
        try:
            raridade = page.locator('//*[@id="details-screen-rarity"]/a[1]').inner_text()
        except Exception:
            raridade = "N/A"
            
        # 4. Preço
        try:
            preco_medio = page.locator('xpath=//*[@id="container-price-mkp-card"]/div[2]/div[2]/div[2]/div').inner_text()
        except Exception:
            preco_medio = "N/A"

        # 5. Nomes (Tenta extrair, usa fallback se falhar)
        try:
            nome_por = page.locator('xpath=/html/body/main/div[1]/div[3]/div[2]/div/div/div[2]/div[1]/div[1]').inner_text(timeout=3000)
            nome_ing = page.locator('xpath=/html/body/main/div[1]/div[3]/div[2]/div/div/div[2]/div[1]/div[2]').inner_text(timeout=3000)
        except Exception:
    ...
    if len(botoes_edicao) == 0:
        print(f"   [Info] Apenas 1 edição encontrada (sem slider). Extraindo dados da tela principal.")
        ...
        for indice, botao in enumerate(botoes_edicao, start=1):
            try:
                ...
                page.wait_for_selector('div.name-edition', state='visible', timeout=5000)
...
for portugues, ingles in zip(
                        cartas_df['nome_portugues'], cartas_df['nome_ingles']):
...
if todos_os_dados:
    ...
    cartas_web_df = cartas_web_df.rename(columns={
    ...
    cartas_df['edicao'] = cartas_df['edicao'].replace('Máscara de Mercádia', 'Máscaras de Mercádia')
